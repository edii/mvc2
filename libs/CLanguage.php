...
//        }
        ...
        protected function setDefaultLang() {
            ...
            if(empty( $this ->_defaultlang )) {
                ...
                $_query = self::$db -> query( "SELECT `id` as `lang_id`, 
                                                    `code`, 
                                                    `name`, 
                                                    `default`, 
                                                    `icon`,
                                            ...
                                            WHERE `id` <> 0 
                                                  AND `hidden` = 0
                                                  AND `default` = 1 ORDER BY id ASC LIMIT 1") -> fetchAll();
                if(is_array($_query ) and count($_query) > 0) {
                    foreach($_query as $_lang) :
                        ...
                        $this ->_defaultlang[ $_lang->code ] = (array)$_lang;
                    endforeach;
                }
        ...
        protected function setLanguages() {
            if(empty( $this -> _languages )) {
                $query = self::$db -> query( "SELECT `id` as `lang_id`, 
                                                    `code`, 
                                                    `name`, 
                                                    `default`, 
                                                    `icon`,
                                                    ...
                                                    `locale`
                                                FROM `language` 
                                                WHERE `id` <> 0 AND `hidden` = 0") -> fetchAll();
                
                
                if(is_array($query) and count($query) > 0) {
                    foreach($query as $_item):
                        $this -> _languages[$_item -> code] = (array)$_item;
                    endforeach;
                }
            }
            return $this -> _languages;
