...
//                            $route=$owner->defaultController;
        ...
        protected function getLangs( $_code = false ) {
            ...
            $_where = "";
            if($_code_lang) {
                $_where .= " AND `code` = '".$_code_lang."'";
            } else {
                 $_where .= " AND `default` = 1";
            }
            
            
            $_q_lang = $_db -> query( "SELECT `id` as `lang_id`, 
                                                  `code`, 
                                                  `name`, 
                                                  `default`, 
                                                  `icon`,
                                                  ...
                                                  `locale`
                                          FROM `language` 
                                          WHERE `id` <> 0 AND `hidden` = 0 ". $_where) -> fetchAll();
            
            if(is_array($_q_lang) and count($_q_lang) >0 ) {
                $db_lang = $_q_lang;
            } else {
                return $_db -> query( "SELECT `id` as `lang_id`, 
                                                  `code`, 
                                                  `name`, 
                                                  `default`, 
                                                  `icon`,
                                                  ...
                                                  `locale`
                                          FROM `language` 
                                          WHERE `id` <> 0 AND `hidden` = 0 AND `default` = 1 ORDER BY id ASC LIMIT 1") -> fetchAll();
            }
