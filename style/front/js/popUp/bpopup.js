...
//            console.log( wW, wH );
        ...
        function open(){
            ...
            if (o.modal) {
                ...
                .css({backgroundColor: o.modalColor, 
                        position: 'fixed', 
                        top: 0, 
                        right:0, 
                        bottom:0, 
...
//            console.log( (hPos + width) *-1 );
	...
	function recenter(content){
        ...
        function bindEvents() {
            ...
            if($popup.find('.bClose').length) {
                $popup.delegate('.bClose', 'click.'+id, close);
            } else
                $popup.delegate('.' + o.closeClass, 'click.'+id, close); // legacy, still supporting the close class bClose
        ...
        function unbindEvents() {
            ...
            if($popup.find('.bClose').length) {
                $popup.undelegate('.bClose', 'click.'+id, close).data('bPopup', null);
            } else
                $popup.undelegate('.' + o.closeClass, 'click.'+id, close).data('bPopup', null);
        };
		
	function doTransition(open) {
            
//            console.log(' --------------------- \\ doTransition -------------------');
//            console.log(!(!o.follow[0] && fixedHPos || fixedPosStyle));
//            console.log( getLeftPos(!(!o.follow[0] && fixedHPos || fixedPosStyle)) );
...
//            console.log( hPos + d.scrollLeft() );
	...
	function triggerCall(func, arg) {
       	...
       	function calcPosition(){
            var _pwW = (o.inElement) ? (wW - $popup.find( o.inObjectClass ).outerWidth(true)) / 2 : (wW - $popup.outerWidth(true)) / 2;
            // inElement:        false
            // , inObjectClass:    '.popUp'          
//            console.log(' --------------------- \\ calcPosition -------------------');
//            console.log( _pwW );
//            console.log(wW, $popup.outerWidth(true));
            
            vPos        = fixedVPos ? o.position[1] : Math.max(0, ((wH- $popup.outerHeight(true)) / 2) - o.amsl)
            , hPos      = fixedHPos ? o.position[0] : _pwW
            , inside    = insideWindow();
        };
